...
import email_manager.feedparser
from email_manager import header
import email_manager.message
import email_manager.utils
import errno
...
def _entry_disposition(response_headers, request_headers):
    ...
    if (
        ...
        date = calendar.timegm(email_manager.utils.parsedate_tz(response_headers["date"]))
        now = time.time()
        ...
        if "max-age" in cc_response:
            ...
            expires = email_manager.utils.parsedate_tz(response_headers["expires"])
            if None == expires:
                freshness_lifetime = 0
            else:
...
def _updateCache(request_headers, response_headers, content, cache, cachekey):
    if cachekey:
        ...
        if "no-store" in cc or "no-store" in cc_response:
            ...
            info = email_manager.message.Message()
            ...
            if self.cache:
                ...
                if cached_value:
                    try:
                        ...
                        info = email_manager.message_from_bytes(info)
                        for k, v in info.items():
                            if v.startswith("=?") and v.endswith("?="):
                                info.replace_header(
                                    k, str(*email_manager.header.decode_header(v)[0])
                                )
...
class Response(dict):
    ...
    def __init__(self, info):
        ...
        elif isinstance(info, email_manager.message.Message):
