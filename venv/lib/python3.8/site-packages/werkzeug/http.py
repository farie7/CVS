...
def parse_date(value: t.Optional[str]) -> t.Optional[datetime]:
    ...
    try:
        dt = email_manager.utils.parsedate_to_datetime(value)
...
def http_date(
    ...
    if isinstance(timestamp, date):
        ...
        return email_manager.utils.format_datetime(timestamp, usegmt=True)

    if isinstance(timestamp, struct_time):
        timestamp = mktime(timestamp)

    return email_manager.utils.formatdate(timestamp, usegmt=True)
